...
local function ShouldStandStill(inst)
    return inst:HasTag("pet_hound") or inst:HasTag("companion") and not TheWorld.state.isday and not GetLeader(inst) and not inst.components.combat.target and inst:IsNear(GetHome(inst), SIT_BOY_DIST)
end
...
function HoundBrain:OnStart()
    ...
    local root = PriorityNode(
    {
        ...
        WhileNode(function() return self.inst:HasTag("pet_hound") or self.inst:HasTag("companion") end, "Is Pet", ChaseAndAttack(self.inst, 10)),
        WhileNode(function() return not self.inst:HasTag("pet_hound") or self.inst:HasTag("companion") and GetHome(self.inst) end, "No Pet Has Home", ChaseAndAttack(self.inst, 10, 20)),
        WhileNode(function() return not self.inst:HasTag("pet_hound") or self.inst:HasTag("companion") and not GetHome(self.inst) end, "Not Pet", ChaseAndAttack(self.inst, 100)),
